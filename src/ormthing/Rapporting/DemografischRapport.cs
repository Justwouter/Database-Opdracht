...
class DemografischRapport : Rapport
{
    ...
    private async Task<bool> AlleGastenHebbenReservering() => await Task<bool>.Run(() =>{return context.Guests.Where<Gast>(gast => gast.reserveringen.Count() > 0).Count() == context.Guests.Count();});
